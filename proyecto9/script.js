...
document.querySelector("form").addEventListener("submit", e =>{
    e.preventDefault(); /*poner formulario en blanco*/
    agregarEvento();
...
function mostrarEventos(){
    const eventosHMTL = eventos.map((evento) =>{
        return`
            <div class="evento">
                <div class="dias">
    ...
    listaEventos.innerHTML = eventosHMTL.join("");
    document.querySelectorAll('.eliminar').forEach(button => {
        button.addEventListener("click", e => {
            const id = button.getAttribute('data-id');
            eventos = eventos.filter(evento => evento.id !== id);/*filtro para que elimine un evento*/

            guardar(JSON.stringify(eventos)); /*eliminar eventos del localStorage */
